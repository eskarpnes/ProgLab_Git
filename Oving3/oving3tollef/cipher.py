...
class Cipher:
	...
	def verify(self, original, from_receiver):
		print ('Checking if',original,'is equal to',from_receiver)
...
class Unbreakable(Cipher):
	...
	def generate_keys(self):
		# return 'PIZZA'
		key = random.choice(self.word_list)
	...
	def encode(self, txt, key):
		...
		for i in range(len(txt)):
			...
			# print (sum)
			sum = sum % self.HIGH
			# print (c, ord(c)-self.LOW,'\t->\t', k, ord(k)-self.LOW, '\tModSum:',sum-self.LOW,'\t',chr(sum))
			encoded += chr(sum)
		print ('Encoded:',encoded)
		return encoded
			
	def decode(self, txt, key):
		# print ('decoding',txt,'with key:',key)
		decrypt, decoded = '', ''
		print ('Finding decrypting key for',key,end = '...')
		for c in key:
			# print (c, ord(c))
			new = (self.M - (ord(c)-self.LOW)) % self.HIGH # (M-n) mod M
			decrypt += chr(new)
		print (decrypt)
		for i in range (len(txt)):
			decrypt_val = ord(decrypt[i % len(decrypt)]) # ascii value of the code index
			txt_val = ord(txt[i]) # ascii value of the current letter
			new = (txt_val + decrypt_val) % self.HIGH
			# print ('ascii:', new)
			# print (txt_val,'+',decrypt_val,'mod',self.M,'=',new-1,chr(new-1))
			decoded += chr(new)
